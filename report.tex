...
\[
  [\texttt{A5 5A}][\texttt{session\_id}][\texttt{packet\_index}][\texttt{packet\_count}][\texttt{payload\_len}][\texttt{payload}][\texttt{crc16}].
\]
\[
\begin{array}{l|c|c|c|c|c|c|c}
\textbf{Field} & \texttt{SYNC} & \texttt{SESSION} & \texttt{PACKET\_IDX} & \texttt{PACKET\_CNT} & \texttt{PL\_LEN} & \texttt{PAYLOAD} \;|\; \texttt{CRC16} \\
\hline
\textbf{Length} & 2\text{ B} & 2\text{ B} & 2\text{ B} & 2\text{ B} & 1\text{ B} & 0\text{--}64\text{ B} \;|\; 2\text{ B}
\end{array}
\]
...
\begin{sloppypar}
On our test machine, the threshold tuner consistently produced two clusters. A short calibration log is included with the submission in \nolinkurl{report_assets/threshold_measurements.txt}, and the raw samples behind the paper-style distribution in Figure~\ref{fig:threshold-dist} are saved in \nolinkurl{report_assets/threshold_distribution.csv}. In the later portion of the tuner log, the non-flushed average settles around \SIrange{360}{370}{cycles}, while the flushed average settles around \SIrange{815}{820}{cycles}. We therefore used \SI{600}{cycles} as the receiver threshold. This choice is conservative, but it sits in the gap between the two groups and worked well in our tests.
\end{sloppypar}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\linewidth]{report_assets/threshold_distribution.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.90\linewidth]{report_assets/probe_trace.png}
...
For the packetized measurements below, we used the message \texttt{Packetized covert channels survive duplicates and CRC checks.}, which is 61 bytes long. With a 16-byte payload size, that message becomes four packets. One full transmission cycle contains 105 bytes on the wire, or 840 transmitted bits. Since the message itself carries 488 payload bits, the ideal cycle-limited payload goodput is
\[
R_{\text{ideal}} = \frac{488}{840d},
\]
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}rrrrl@{}}
        ...
        \midrule
        10 & 58.1 & 9.19 & 63.2 & Stable\\
        5  & 116.2 & 4.66 & 124.7 & Stable\\
        2  & 290.5 & 2.11 & 275.3 & Works Well\\
        1  & 581.0 & 1.69 & 343.8 & Higher variance\\
        \bottomrule
    \end{tabular}%
    }
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.80\linewidth]{report_assets/bandwidth_sweep.png}
